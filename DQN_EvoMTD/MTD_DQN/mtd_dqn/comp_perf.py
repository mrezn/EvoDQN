"""Comparative performance export utilities for WF–MTD style models."""
...
@dataclass
class StepLog:
    """Single-step diagnostic record used for comparative perf exports."""
...
def compute_comp_perf(
    ...
    if not df.empty:
        ...
        ud_sal_path = tables_dir / "ud_sal_over_time.csv"
        ud_sal_cols = ["t"]
        if "UD" in df:
            ud_sal_cols.append("UD")
        if "SAL" in df:
            ud_sal_cols.append("SAL")
        df[ud_sal_cols].to_csv(ud_sal_path, index=False)
